...
>>> hp = HydPy('LahnH')
>>> pub.timegrids = '1996-01-01', '1996-02-05', '1d'
...
>>> pub.timegrids.sim.lastdate = '1996-02-05'
>>> hp.simulate()
...
>>> for node_ in hp.nodes:
...     filepath = f'../openda_projects/AEnKF/data/{node_.name}.discharge.noos'
...     with open(filepath, 'w') as noosfile:
...         _ = noosfile.write('# TimeZone:GMT+1\n')
...         for date, discharge in zip(pub.timegrids.init, node_.sequences.sim.series):
...             date = date + '1d'
...
>>> _ = run_subprocess('oda_run_batch main.oda', verbose=True)
>>> import runpy
>>> results = runpy.run_path('results/final.py')
>>> sim_corrected = results['pred_f_central'][:, 5:10].flatten()
...
>>> sim_members = [results[f'pred_f_{idx}'][:, 5:10].flatten() for idx in range(50)]
>>> from matplotlib import pyplot
>>> _ = pyplot.plot(sim_members[0], 'grey', label='members')
>>> for sim_member in sim_members[1:]:
...     _ = pyplot.plot(sim_member, 'grey')
