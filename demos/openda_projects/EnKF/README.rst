...
does not only perturb some model properties randomly but to improve the
quality of the simulation.  There is also a similarity to the
...
the Ensemble Kalmanfilter to be able to adapt another simulation run
to this distortion.

Prepare the artificial data
...........................

The precedure to prepare the `LahnH`_ project should be well known by know:
...
We now perform two consequitive simulation runs, the first one throughout
...
The disturbed series of `LZ`_ looks as follows:
...
we will take as the "true" discharge in the following, on sees
an abrupt increase due the sudden increase of ground water:
...
We now reset the previously saved model states (with the undisturbed
...
The Ensemble Kalmanfilter needs to know (an estimate of) the "true"
...
The `OpenDA`_ configuration resembles the one of the `multiple sequential runs`_
example.  Notable differences are the that file `main.oda`_ selects the
Ensemble Kalmanfilter algorithm, file `algorithm.xml`_ enables not only
`stochInit` but also `stochForcing` to allow for constantly applied
state perturbations, and files `model.xml`_ and `hydpy.xml`_ define an
exchange item for changing the values of state `LZ`_ instead of parameter
`Alpha`_.  Using 20 ensemble members and the AR(1) model for generating
the stochastic perturbations as defined in file `algorithm.xml`_, the
Ensemble Kalmanfilter yields the following corrected discharge series:
...
by the Ensemble Kalman filter approximates the "true" discharge much
more than the uncorrected discharge:
